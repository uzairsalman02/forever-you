...
// Curated list of high quality aesthetic Unsplash photos for polaroid collage
const sampleImages = [
...
// Generate 48 distinct polaroid memory items
export const VAULT_MEMORIES: VaultMemory[] = Array.from({ length: 48 }).map(
  (_, i) => {
    // Generate deterministic rotation between -15deg and +15deg
    const angle = ((i * 7 + 3) % 31) - 15;
